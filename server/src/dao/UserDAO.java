...
public class UserDAO extends DAO {
    ...
    public void addUser(User user) {
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("INSERT INTO user(username, password, nickname, avatar)\n"
                    + "VALUES(?,?,?,?)");
            preparedStatement.setString(1, user.getUsername());
            preparedStatement.setString(2, user.getPassword());
    ...
    public boolean checkDuplicated(String username) {
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("SELECT * FROM user WHERE username = ?");
    ...
    public boolean checkIsBanned(User user) {
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("SELECT * FROM banned_user WHERE ID_User = ?");
    ...
    public void updateBannedStatus(User user, boolean ban) {
        try {
            PreparedStatement preparedStatement1 = dbConnection.prepareStatement("INSERT INTO `banned_user`(`ID_User`) VALUES (?)");
            PreparedStatement preparedStatement2 = dbConnection.prepareStatement("DELETE FROM `banned_user` WHERE ID_User=?");
            if (ban) {
    ...
    public void updateToOnline(int ID) {
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("UPDATE user\n"
                    + "SET IsOnline = 1\n"
                    + "WHERE ID = ?");
    ...
    public List<User> getListFriend(int ID) {
        ...
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("SELECT User.ID, User.NickName, User.IsOnline, User.IsPlaying\n"
                    + "FROM user\n"
                    + "WHERE User.ID IN (\n"
                    + "	SELECT ID_User1\n"
                    + "    FROM friend\n"
    ...
    public boolean checkIsFriend(int ID1, int ID2) {
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("SELECT Friend.ID_User1\n"
                    + "FROM friend\n"
    ...
    public void addFriendShip(int ID1, int ID2) {
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("INSERT INTO friend(ID_User1, ID_User2)\n" +
                    "VALUES (?,?)");
            preparedStatement.setInt(1, ID1);
            preparedStatement.setInt(2, ID2);
            System.out.println(preparedStatement);
    ...
    public void removeFriendship(int ID1, int ID2) {
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("DELETE FROM friend\n" +
                    "WHERE (ID_User1 = ? AND ID_User2 = ?)\n" +
                    "OR(ID_User1 = ? AND ID_User2 = ?)");
            preparedStatement.setInt(1, ID1);
            preparedStatement.setInt(2, ID2);
    ...
    public int getRank(int ID) {
        int rank = 1;
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("SELECT user.ID\n"
                    + "FROM user\n"
    ...
    public int getNumberOfWin(int ID) {
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("SELECT user.NumberOfWin\n"
                    + "FROM user\n"
                    + "WHERE user.ID = ?");
            preparedStatement.setInt(1, ID);
            ResultSet rs = preparedStatement.executeQuery();
    ...
    public int getNumberOfDraw(int ID) {
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("SELECT user.NumberOfDraw\n"
                    + "FROM user\n"
                    + "WHERE user.ID = ?");
            preparedStatement.setInt(1, ID);
            ResultSet rs = preparedStatement.executeQuery();
    ...
    public int getNumberOfGame(int ID) {
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("SELECT user.NumberOfGame\n"
                    + "FROM user\n"
                    + "WHERE user.ID = ?");
            preparedStatement.setInt(1, ID);
            ResultSet rs = preparedStatement.executeQuery();
    ...
    public String getNickNameByID(int ID) {
        try {
            PreparedStatement preparedStatement = dbConnection.prepareStatement("SELECT user.NickName\n"
                    + "FROM user\n"
                    + "WHERE user.ID=?");
            preparedStatement.setInt(1, ID);
            ResultSet rs = preparedStatement.executeQuery();
